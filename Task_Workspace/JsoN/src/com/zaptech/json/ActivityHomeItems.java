...
public class ActivityHomeItems extends Activity implements OnClickListener,
	...
	private boolean flagFirstRun = false;
	private View viewFirstInstall;
	...
	void checkFirstInstall() {
		...
		if (pref.getBoolean("flag", flagFirstRun)) {

			new GetImage().execute();

			Toast.makeText(ActivityHomeItems.this, "Allready installed",
					Toast.LENGTH_SHORT).show();

		} else {

			Toast.makeText(ActivityHomeItems.this, "First time installed",
					Toast.LENGTH_SHORT).show();

			LayoutInflater inflater = (LayoutInflater) ActivityHomeItems.this
					.getSystemService(Context.LAYOUT_INFLATER_SERVICE);

			viewFirstInstall = inflater.inflate(R.layout.raw_first_install,
					null);

			RelativeLayout layout = (RelativeLayout) viewFirstInstall
					.findViewById(R.id.relFirstInstall);
			...
			layout.setOnTouchListener(new OnTouchListener() {
				...
				public boolean onTouch(View v, MotionEvent event) {
					...
					case MotionEvent.ACTION_DOWN:
						Intent in = new Intent(ActivityHomeItems.this,
		...
		try {
			...
			for (int i = 0; i < newsItems.length(); i++) {
				...
				for (int j = 0; j < items.length(); j++) {
					...
					/*
					 * System.err.println("ID========"+newsitem_ItemModel_obj.getId
					 * ());
					 */
					...
					newsitem_ItemModel_obj.setListIcon(items_obj
							.getString("listIcon"));
					newsItemModel_obj.setObj_item(newsitem_ItemModel_obj);

					JSONObject newsimageObj = items_obj
							.getJSONObject("newsImage");

					NewsImage_Model newsImageModel_obj = new NewsImage_Model();
