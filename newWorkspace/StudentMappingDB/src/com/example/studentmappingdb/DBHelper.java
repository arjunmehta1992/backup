...
public class DBHelper extends SQLiteOpenHelper {
	...
	public ArrayList<String> GetCollegeNameFromStdName(String stdname) {
		...
		Cursor cursor = db.rawQuery(
				"SELECT college_id from student_master where std_name='"
						+ stdname + "'", null);
		Log.i("total=", " >>>>>>>>>>>>>>>>>>>>" + cursor.getCount());
		if (cursor != null) {
			if (cursor.moveToFirst()) {
				do {

					int a = cursor.getInt(cursor
							.getColumnIndex(STUDENT_COLUMN_ID));
	...
	public List<String> getAllLabels() {
		...
		String selectQuery = "SELECT  * FROM " + COLLEGE_TABLE_NAME;
	...
	public void deleteStudent(String collegename, String stdName) {
		...
		if (cursor != null) {
			if (cursor.moveToFirst()) {
				id = cursor.getInt(cursor.getColumnIndex(COLLEGE_COLUMN_ID));
			}
		}
		...
		// Cursor a=db.rawQuery("delete from "+STUDENT_TABLE_NAME + " where " +
		// STUDENT_COLUMN_NAME + "='" + stdName +
		// "' and "+STUDENT_COLUMN_COLLEGE_ID + "=" + id, null);
		db.delete(STUDENT_TABLE_NAME, STUDENT_COLUMN_NAME + "='" + stdName
				+ "' AND " + STUDENT_COLUMN_COLLEGE_ID + "=" + id, null);
	...
	public void updateCollege(CollegeData clg,String tempClgName) {

		SQLiteDatabase db = this.getWritableDatabase();
		ContentValues values = new ContentValues();
		values.put(COLLEGE_COLUMN_NAME, clg.getCollege_name());
		values.put(COLLEGE_COLUMN_ADDRESS, clg.getCollege_address());
		values.put(COLLEGE_COLUMN_CONTACT, clg.getCollege_contact());
		values.put(COLLEGE_COLUMN_LATTITUDE, clg.getCollege_lat());
		values.put(COLLEGE_COLUMN_LONGITUDE, clg.getCollege_long());

		
		db.update(COLLEGE_TABLE_NAME, values,COLLEGE_COLUMN_NAME+"='"+tempClgName+"'" ,null);
		
		Log.i("ID=========", ">>>>>>>>>>>>>>"+clg.getCollege_name());
		db.close();
	}

	public void updateStudent(StudentData std,String std_obj,CollegeData obj_clg)
	{
		SQLiteDatabase db = this.getWritableDatabase();
		Cursor cursor=db.rawQuery("select "+ COLLEGE_COLUMN_ID + " from " + COLLEGE_TABLE_NAME + " where "+ COLLEGE_COLUMN_NAME + "='" + obj_clg.getCollege_name() +"'", null);
		int clg_id=0;
		if(cursor!=null)
		{
			if(cursor.moveToFirst())
			{
				...
				clg_id=cursor.getInt(cursor.getColumnIndex(COLLEGE_COLUMN_ID));
				} while (cursor.moveToNext());
			}
		}
		
		ContentValues values = new ContentValues();
		values.put(STUDENT_COLUMN_NAME, std.getStd_name());
		values.put(STUDENT_COLUMN_ADDRESS, std.getStd_address());
		values.put(STUDENT_COLUMN_CONTACT, std.getStd_contact());
		values.put(STUDENT_COLUMN_EMAIL, std.getStd_email());
		values.put(STUDENT_COLUMN_GENDER, std.getStd_gender());
		values.put(STUDENT_COLUMN_COLLEGE_ID,clg_id);

		
		db.update(STUDENT_TABLE_NAME, values,STUDENT_COLUMN_NAME+"='"+std_obj+"'" ,null);
